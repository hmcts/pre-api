...
class CaptureSessionManager:
    ...
    def check_record_in_temp_table(self, destination_cursor, recording_id):
        destination_cursor.execute("SELECT EXISTS (SELECT 1 FROM public.temp_recordings WHERE recording_id=%s)", (recording_id,))
        ...
        for temp_recording in temp_recording_data:
            ...
            if status is None:
                self.failed_imports.append({
                    'table_name': 'capture_sessions',
                    'table_id': recording_id,
                    'details': f'No valid status for recording id: {recording_id}'
                })
                continue
            ...
            started_at_datetime, user_email_started = self.get_recording_date_and_user(
                recording_id, "Start")
            started_by_user_id = get_user_id(
                destination_cursor, user_email_started)
            started_at = started_at_datetime if started_at_datetime else created_at

            finished_at_datetime, user_email_finished = self.get_recording_date_and_user(
                recording_id, "Stop")
